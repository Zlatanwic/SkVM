...
export async function runPipeline(config: PipelineConfig): Promise<void> {
  ...
  if (!(await pipelineSkillFile.exists())) {
    // Not a UsageError: fires after RunSession.start (and possibly a full
    // profile run) — past the pre-side-effect window flags.ts's contract
    // requires for UsageError throws.
