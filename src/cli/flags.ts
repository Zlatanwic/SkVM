 ...
 * - An empty value (`--key=`) is treated as "flag not provided", matching the
 *   truthiness checks the legacy handlers used.
 *
...
export function defineFlags<const S extends FlagSpecs>(
  ...
  function help(): string {
    const rows: Array<[string, string]> = dataEntries.map(([name, s]) => {
      ...
      // Declared defaults render automatically so flag authors never repeat
      // them in prose (the drift this layer exists to kill). Bool defaults
      // stay silent — absent simply means false.
      let text = s.help ?? ""
    ...
    const lines = [`skvm ${command} - ${summary}`, "", "Options:"]
  ...
  function parse(argv: string[]): ParseResult<S> {
    ...
    for (const [name, s] of dataEntries) {
      // Empty value (`--key=`) counts as "not provided", matching the
      // truthiness semantics of the legacy handlers.
      let value: string | undefined = raw[name] === "" ? undefined : raw[name]
      if (value === undefined) {
